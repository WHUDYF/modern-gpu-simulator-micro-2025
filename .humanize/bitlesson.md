...
Root Cause: Gate0 accepted a handwritten attestation over the same untrusted root, restart/revalidation skipped live collector session binding when `active_collector_session_id` was absent, Gate0 later treated every no-active-session recording as revalidation requiring an existing manifest, and a later attempted relaxation let root-local evidence fields or an active session ID alone mint a first formal manifest without a process-owned collector session record.
Solution: Bind Gate0 first-time formal acceptance to acquisition-produced session/attestation artifacts plus an active collector session record containing the session hash and output root, require no-active-session Gate0 revalidation to be backed by an existing formal manifest whose evidence/source hashes still match, and make Gate6 formal validation load Gate5 lineage and manifest files from a persisted artifact root before recomputing hashes.
Constraints: This does not prove real ResNet-50 trace availability; fixture-backed producer-path tests remain contract coverage only; no-active-session restart/revalidation is allowed only for an already formalized root, not for minting a first formal manifest from self-authenticated files; process-local active session IDs without the recorded session hash/root are not trusted.
