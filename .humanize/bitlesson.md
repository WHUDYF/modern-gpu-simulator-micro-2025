...
Source Rounds: 33, 34, 38, 44, 48, 50, 51

## Lesson: gate2-ncu-metric-coverage
Lesson ID: BL-20260609-gate2-ncu-metric-coverage
Scope: experiments/baseline_diagnosis/m1_ncu_capture_dispatcher.py, experiments/baseline_diagnosis/shared_acquisition.py, experiments/baseline_diagnosis/tests/test_m1_ncu_capture_dispatcher.py
Problem Description: Gate2 can proceed to NCU capture when Nsight Compute resolves only a subset of the required canonical feature metrics.
Root Cause: The dispatcher treated `any(selected_for_ncu_metrics)` as sufficient coverage, but the downstream measured feature extractor requires all non-launch-metadata canonical feature metrics.
Solution: Preflight the full metric resolution table and block Gate2 with `required_feature_metrics_incomplete` when any required NCU-backed feature metric is unresolved; keep `selected_metrics_empty` for the zero-selected case.
Constraints: Launch metadata rows such as `num_thread_blocks` are not NCU metric requirements; timing metrics remain additional capture metrics and do not replace canonical feature coverage.
Validation Evidence: `pytest -q experiments/baseline_diagnosis/tests/test_m1_ncu_capture_dispatcher.py` passed with 7 tests; `python3 -m py_compile experiments/baseline_diagnosis/m1_ncu_capture_dispatcher.py experiments/baseline_diagnosis/tests/test_m1_ncu_capture_dispatcher.py`; `git diff --check && git diff --cached --check`.
Source Rounds: 51
...
## Lesson: replay-boundaries-authenticate-before-deriving
Lesson ID: BL-20260610-replay-boundaries-authenticate-before-deriving
Scope: experiments/gcl_phase_a/pipeline.py, experiments/gcl_phase_b/pipeline.py, experiments/gcl_phase_b/resnet50_gate_pipeline.py, tests/gcl_phase_a/test_pipeline.py, tests/gcl_phase_b/test_replay.py
Problem Description: From-disk repair and replay paths can accept attacker-controlled artifacts when they deserialize checkpoints with pickle semantics or derive expected replay provenance from an unauthenticated manifest field.
Root Cause: Repair loaders used `torch.load(..., weights_only=False)` on checkpoint files from replay directories, and Phase B replay validated augmentation manifests against hash self-consistency before checking that each view matched the seed/order/view hash generated by the authenticated pipeline run.
Solution: Load checkpoint files only through `torch.load(weights_only=True)`, fail on old torch versions that lack safe loading, verify `pipeline_manifest_hash` before using manifest fields such as `seed`, and recompute expected per-view augmentation manifests from the tensors and seed during replay.
Constraints: Checkpoint files remain PyTorch state bundles; replay rejects tampered augmentation view provenance even when the attacker recomputes augmentation, bundle, and pipeline hashes; path mismatch checks still run before manifest hash checks.
Validation Evidence: `pytest -q tests/gcl_phase_a/test_pipeline.py::test_embedding_export_stage_from_disk_loads_checkpoint_weights_only`; `pytest -q tests/gcl_phase_b/test_replay.py::test_phase_b_replay_rejects_rewritten_augmentation_view_provenance_after_hash_refresh tests/gcl_phase_b/test_replay.py::test_phase_b_replay_rejects_stale_resource_blocked_pipeline_manifest`; `pytest -q tests/gcl_phase_a/test_pipeline.py tests/gcl_phase_b/test_replay.py tests/gcl_phase_b/test_resnet50_gate_pipeline.py`; `python3 -m py_compile experiments/gcl_phase_a/pipeline.py experiments/gcl_phase_b/pipeline.py experiments/gcl_phase_b/resnet50_gate_pipeline.py tests/gcl_phase_a/test_pipeline.py tests/gcl_phase_b/test_replay.py`.
Source Rounds: 56

## Lesson: formal-trace-identity-and-protobuf-shape
Lesson ID: BL-20260610-formal-trace-identity-protobuf-shape
Scope: experiments/gcl_phase_b/resnet50_adapter.py, experiments/gcl_phase_b/resnet50_gate0.py, tests/gcl_resnet50/test_gate1_adapter.py, tests/gcl_resnet50/test_gate0_trace_acquisition.py
Problem Description: Formal ResNet-50 replay can widen a bounded canonical invocation slice when scheduler metadata only has legacy kernel IDs, and Gate0 can mark structurally empty protobuf artifacts as formal because protobuf parsing succeeds on empty messages.
Root Cause: Legacy scheduler IDs collide across repeated launches of the same kernel ID, so filtering scheduler rows by a legacy-ID union keeps multiple launches; `ParseFromString()` validates wire format but not required semantic content such as kernel invocations, warps, or instructions.
Solution: For legacy-only scheduler metadata, filter scheduler rows by the launch-order positions selected by the filtered dynamic trace instead of legacy IDs; after protobuf parsing, require dynamic traces to contain at least one kernel and each threadblock protobuf to contain warp and instruction records.
Constraints: Explicit legacy alias requests keep the existing first-match dynamic-trace behavior; invocation_limit prefix filtering for legacy scheduler metadata remains prefix-based; minimal test protobuf helpers must now write one warp instruction to represent a structurally usable threadblock.
Validation Evidence: `pytest -q tests/gcl_resnet50/test_gate1_adapter.py::test_gate1_canonical_invocation_ids_filter_legacy_scheduler_repeated_kernel tests/gcl_resnet50/test_gate1_adapter.py::test_gate1_formal_pb_uses_launch_order_invocation_ids_for_reused_kernel_id tests/gcl_resnet50/test_gate1_adapter.py::test_gate1_invocation_limit_does_not_widen_repeated_kernel_fallback_scheduler tests/gcl_resnet50/test_gate1_adapter.py::test_gate1_legacy_invocation_alias_selects_single_repeated_launch`; `pytest -q tests/gcl_resnet50/test_gate0_trace_acquisition.py::test_gate0_rejects_empty_dynamic_trace_before_formal_manifest tests/gcl_resnet50/test_gate0_trace_acquisition.py::test_gate0_rejects_empty_threadblock_before_formal_manifest tests/gcl_resnet50/test_gate0_trace_acquisition.py::test_gate0_rejects_unreadable_dynamic_trace_before_formal_manifest tests/gcl_resnet50/test_gate0_trace_acquisition.py::test_gate0_rejects_unreadable_threadblock_before_formal_manifest`; `pytest -q tests/gcl_resnet50/test_gate1_adapter.py tests/gcl_resnet50/test_gate0_trace_acquisition.py tests/gcl_phase_b/test_resnet50_adapter.py`; `python3 -m py_compile experiments/gcl_phase_b/resnet50_adapter.py experiments/gcl_phase_b/resnet50_gate0.py tests/gcl_resnet50/test_gate1_adapter.py tests/gcl_resnet50/test_gate0_trace_acquisition.py`.
Source Rounds: 57

## Lesson: disk-repair-stage-boundaries-commit-before-next-stage
Lesson ID: BL-20260610-disk-repair-stage-boundaries
Scope: experiments/gcl_phase_b/pipeline.py, tests/gcl_phase_b/test_pipeline.py
Problem Description: A disk repair helper can leave a replay root internally inconsistent when one stage succeeds, writes some side artifacts, and the next stage fails before core artifacts and manifest hashes are updated.
Root Cause: Phase B embedding export rewrote `rgcn_checkpoint.pt`, readout, lineage, and export report files before selector execution, but delayed `checkpoint_manifest.json`, `embedding_table.json`, augmentation bundle, and pipeline hash updates until selector success.
Solution: Persist Gate5 core artifacts immediately after successful re-export, refresh Gate5 and augmentation hashes from disk when selector is resource-blocked, and update the pipeline seed to the seed used for the re-export before replay validation derives augmentation provenance.
