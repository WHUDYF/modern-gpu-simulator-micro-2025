...
|------|-----------|--------|-----|-------|-------|
| Gate0 real ResNet-50 NVBit trace acquisition and formal acquisition manifest | AC-1 | pending | coding | claude | Gate0 now requires a persisted `nvbit_collector_attestation.json`, but no verified in-workspace real ResNet-50 NVBit acquisition exists yet, no acquisition path writes that artifact, and a synthetic helper root can still self-certify into a formal manifest by adding a matching handwritten attestation over the synthetic artifacts. |
...
| Gate6 selector / representative anchor / post-clustering family evidence | AC-6 | pending | coding | claude | Gate6 now requires a lineage bundle and `gate5_manifests`, but it still accepts forged self-consistent manifest dicts supplied by the caller because it does not load persisted Gate5 manifest files from disk or bind the selector input to those persisted objects. |
| Gate7 correctness evaluation plus Gate8 / Gate9 extension gates | AC-7 | pending | coding | claude | The successful pipeline manifest filename is now consistent, but the current tested success path still relies on synthetic artifact-shape roots rather than a real Gate0/Gate1 acquisition, and the end-to-end baseline-backed pipeline success test removed in Round 5 has still not been restored. |
...
|-------|-----------------|-------------|-----------------|
| Gate0 no longer treats default fixtures as formal and now exposes a runner entrypoint, but no verified in-workspace real ResNet-50 NVBit acquisition, scheduler metadata capture, or collected production artifact set exists yet. Round 6's new persisted attestation remains forgeable because Gate0 accepts any handwritten `nvbit_collector_attestation.json` whose contents and hashes match the current synthetic artifacts, and the acquisition path still does not write that attestation artifact itself. | 6 | AC-1 | Execute and validate the actual Gate0 collection path against a real ResNet-50 NVBit run, implement collector-side emission of `nvbit_collector_attestation.json`, and bind formal acceptance to independently produced acquisition metadata rather than a handwritten attestation over the same untrusted root. |
...
| Gate6 now requires a lineage bundle object and caller-supplied manifest objects, but Round 6's new validation is still self-forgeable because it hashes whichever dicts the caller passes instead of loading the actual persisted Gate5 manifests from disk. | 6 | AC-6 | Make Gate6 load and hash the actual persisted Gate5 manifests (`rgcn_training_run_manifest.json`, `rgcn_checkpoint_manifest.json`, `readout_manifest.json`, and `embedding_export_report.json`) from a persisted artifact root, reject any selector input that is not bound to those files, and add a negative test for a forged table plus forged lineage bundle plus forged manifest-dict set. |
| The successful pipeline manifest now uses `gate1_7_pipeline_manifest.json`, but the tested success path still does not use a real Gate0/Gate1 acquisition root and the end-to-end baseline-backed pipeline success test removed in Round 5 is still absent. | 6 | AC-7 | Keep the filename fix, restore end-to-end baseline-backed pipeline coverage, and drive the formal success path from a real Gate0/Gate1 acquisition root before treating AC-7 as complete. |
