...
|------|-----------|--------|-----|-------|-------|
| Gate0 real ResNet-50 NVBit trace acquisition and formal acquisition manifest | AC-1 | pending | coding | claude | Gate0 now writes a persisted collector session plus attestation during acquisition, but no verified in-workspace real ResNet-50 NVBit acquisition exists yet, and a synthetic helper root can still self-certify into a formal manifest by adding a handwritten `.nvbit_collector_session.json` plus matching `nvbit_collector_attestation.json` over the synthetic artifacts. |
...
| Gate7 correctness evaluation plus Gate8 / Gate9 extension gates | AC-7 | pending | coding | claude | The baseline-backed Gate7/Gate9 pipeline success test has been restored as debug/report-path coverage, but the tested success path still relies on synthetic artifact-shape roots rather than a real Gate0/Gate1 acquisition, so AC-7 remains incomplete for formal-plan acceptance. |
...
|-------|-----------------|-------------|-----------------|
| Gate0 now writes collector session/attestation artifacts during acquisition, but no verified in-workspace real ResNet-50 NVBit acquisition, scheduler metadata capture, or collected production artifact set exists yet. Round 7's collector boundary remains forgeable because Gate0 accepts a handwritten `.nvbit_collector_session.json` plus matching `nvbit_collector_attestation.json` and evidence fields whose contents and hashes match the current synthetic artifacts. | 7 | AC-1 | Execute and validate the actual Gate0 collection path against a real ResNet-50 NVBit run, and bind formal acceptance to acquisition metadata that a repo-local synthetic helper cannot fabricate after the fact rather than trusting handwritten session/attestation JSON over the same untrusted root. |
...
| The baseline-backed pipeline success test is restored, but only as debug/report-path coverage rooted in synthetic artifact-shape acquisition rather than a real Gate0/Gate1 collection. | 7 | AC-7 | Keep the restored baseline-backed coverage, and drive the formal success path from a real Gate0/Gate1 acquisition root before treating AC-7 as complete. |
