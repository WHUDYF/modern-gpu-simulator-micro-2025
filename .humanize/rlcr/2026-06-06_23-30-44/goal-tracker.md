...
|------|-----------|--------|-----|-------|-------|
| Gate0 real ResNet-50 NVBit trace acquisition and formal acquisition manifest | AC-1 | pending | coding | claude | Gate0 now rejects default synthetic artifact-shape roots, but no verified in-workspace real ResNet-50 NVBit acquisition exists yet, and the new `collector_attestation_hash` check is still forgeable because it is derived entirely from untrusted evidence fields plus the source artifact hashes. |
...
| Gate6 selector / representative anchor / post-clustering family evidence | AC-6 | pending | coding | claude | Gate6 now requires a lineage bundle and `persisted_manifest_hashes`, but those hashes are still accepted when copied directly from a forged embedding table lineage instead of being revalidated against actual persisted Gate5 manifests. |
| Gate7 correctness evaluation plus Gate8 / Gate9 extension gates | AC-7 | pending | coding | claude | The successful pipeline manifest filename is now consistent, but the current tested success path still relies on synthetic artifact-shape roots rather than a real Gate0/Gate1 acquisition, and Round 5 removed the only end-to-end baseline-backed pipeline success test. |
...
|-------|-----------------|-------------|-----------------|
| Gate0 no longer treats default fixtures as formal and now exposes a runner entrypoint, but no verified in-workspace real ResNet-50 NVBit acquisition, scheduler metadata capture, or collected production artifact set exists yet. Round 5's new `collector_attestation_hash` remains self-forgeable because it is computed entirely from untrusted evidence fields plus source artifact hashes. | 5 | AC-1 | Execute and validate the actual Gate0 collection path against a real ResNet-50 NVBit run, and replace the self-declared hash check with validation against a persisted collector-generated attestation artifact that is recomputed from stored acquisition metadata rather than copied from `nvbit_collection_evidence.json`. |
...
| Gate6 now requires a lineage bundle object, but Round 5's new `persisted_manifest_hashes` check is still self-forgeable because it only compares bundle hashes against the same untrusted embedding table lineage fields. | 5 | AC-6 | Make Gate6 load and hash the actual persisted Gate5 manifests (`rgcn_training_run_manifest.json`, `rgcn_checkpoint_manifest.json`, `readout_manifest.json`, and `embedding_export_report.json`) and reject any bundle that does not match those recomputed hashes; add a negative test for a forged table plus forged hash-only bundle pair. |
| The successful pipeline manifest now uses `gate1_7_pipeline_manifest.json`, but the tested success path still does not use a real Gate0/Gate1 acquisition root and Round 5 removed the only end-to-end baseline-backed pipeline success test. | 5 | AC-7 | Keep the filename fix, restore end-to-end baseline-backed pipeline coverage, and drive the formal success path from a real Gate0/Gate1 acquisition root before treating AC-7 as complete. |
