...
|------|-----------|--------|-----|-------|-------|
| Gate0 real ResNet-50 NVBit trace acquisition and formal acquisition manifest | AC-1 | pending | coding | claude | Gate0 now rejects default synthetic artifact-shape roots, but no verified in-workspace real ResNet-50 NVBit acquisition exists yet, and a synthetic helper can still self-certify `evidence_scope = real_resnet50_nvbit_collection` to produce a formal manifest. |
...
| Gate6 selector / representative anchor / post-clustering family evidence | AC-6 | pending | coding | claude | Gate6 now rejects missing lineage and missing lineage bundles, but it still accepts a forged self-consistent embedding table when paired with a forged self-consistent lineage bundle that is not tied to actual persisted Gate5 manifests. |
| Gate7 correctness evaluation plus Gate8 / Gate9 extension gates | AC-7 | pending | coding | claude | The pipeline can now ingest baseline artifacts for Gate7 metric rows and Gate9 comparison, but the current tested success path still relies on a synthetic helper promoted into formal Gate0 scope, and the successful run writes `gate1_5_pipeline_manifest.json` instead of a Gate1-7 manifest. |
...
|-------|-----------------|-------------|-----------------|
| Gate0 no longer treats default fixtures as formal and now exposes a runner entrypoint, but no verified in-workspace real ResNet-50 NVBit acquisition, scheduler metadata capture, or collected production artifact set exists yet. A synthetic helper can still self-certify `evidence_scope = real_resnet50_nvbit_collection` and produce a formal Gate0 manifest. | 4 | AC-1 | Execute and validate the actual Gate0 collection path against a real ResNet-50 NVBit run, and tighten Gate0 so repo-local synthetic helpers cannot opt into formal scope by setting metadata strings. |
...
| Gate6 now requires a lineage bundle object, but it still accepts a forged self-consistent embedding table when paired with a forged self-consistent lineage bundle that is not tied to actual persisted Gate5 manifests. | 4 | AC-6 | Make Gate6 verify lineage against persisted Gate5 artifacts or manifest hashes, and add a negative test for a forged table plus forged lineage bundle pair. |
| The pipeline can now ingest baseline artifacts and run Gate9 comparison, but the tested success path still promotes a synthetic helper into formal Gate0 scope and the successful run writes `gate1_5_pipeline_manifest.json` instead of `gate1_7_pipeline_manifest.json`. | 4 | AC-7 | Keep the baseline-backed path, but drive it from a real Gate0/Gate1 acquisition root and fix the final manifest filename/contract for successful Gate1-7 runs. |
