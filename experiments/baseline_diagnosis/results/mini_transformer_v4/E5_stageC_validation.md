...
虽然内存通道减半，预期降幅应为 ~50%，但实际降幅约 20%。
这是因为这些 kernel 的 DRAM 利用率本身极低（<0.04%），
内存通道并非瓶颈，通道数量减少不会成比例地降低利用率，
而是通过增加延迟轻微压缩了 DRAM 传输效率。方向正确即为有效验证。

**结论：** 模拟器正确响应内存通道数变化（DRAM 吞吐下降）。E4 中 C-2 处方
（residual_add 是 HBM 带宽校准基准）在模拟器层面方向验证通过。
